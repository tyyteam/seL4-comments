...
void activateThread(void)//先schedule再activateThread
{
    ...
    switch (thread_state_get_tsType(NODE_STATE(ksCurThread)->tcbState)) {//riscv的tcbState是ThreadState_Running,然后退出当前函数，进入head.S的la ra, restore_user_context
    ...
    if (NODE_STATE(ksSchedulerAction) != SchedulerAction_ResumeCurrentThread) {//如果不是恢复当前线程
        ...
        if (NODE_STATE(ksSchedulerAction) == SchedulerAction_ChooseNewThread) {
            ...
            if (fastfail &&
                ...
                switchToThread(candidate);//跳转到本文件的441行函数
            }
        }
    }
